...
With the advent of computers which perform millions of floating
...
\end{Def1} According to Requicha and Tilove [5], point set topology provided a
...
hence they were called \textit{\textbf{r -­ sets}} according to Requicha [5]. 
\begin{Def2}
...
\end{Def4} 
\hspace{20}Lazard [9] constructed a Gr\"\o bner basis with  
respect to a term ordering known as the elimination order. Hoffmann [10] 
improved upon a basis conversion algorithm developed in Faugere \textit{et al} [11] by first
...
\end{Def0}
Golub [13] used a numerically stable Jacobi rotation adapted from Abhyankar [12] 
...
exist,   the   earliest   of   which   was   Baumgart's   \textbf{winged ­edge}   data   structure   [16].  
...
holes   (genus). Braid \textit{et al} [17]   modified   the   winged ­edge   data   structure   by  
...
such   as   the  \textbf{half ­edge}   data   structure   by   Mantyla   [19],   Hanharan's   \textbf{face ­edge}  
representation   [20]   and   Ansaldi   \textit {et al}'s   hierarchical   \textbf{face   adjacency}   hypergraph  
...
introduced   by Ansaldi [21].   As   regards   conversion,   Shapiro   and   Vossler [22]   developed  
...
\newgeometry{a4paper,lmargin=2.6in,rmargin=0in,tmargin=2.6in,bmargin=0in}
\begin{figure}[htbp]
\centering
...
importance. Requicha [23] developed   a   new   set   of   operations   peculiar   to  
...
Klinger   and   Dyer [24]   and   was   coined   by   Hunter [25]. It   is   based   on   the  
...
Bentley [26],   hierarchical   data   structures   can   be   used   to   represent   images   in  
...
\hspace{30} The   second   approach   to spacial subdivision due   to   Kawaguchi   and   Endo [27]   is   termed   a  
...
due   to   Hinrichs   and   Nievergelt [28].   In   this   representation,   each   rectangle   is  
...
\textit{R­-tree}   developed   by   Guttman [29]   in   which   each   node   in   the   R-­tree   is   a  
...
\hspace{30} By looking at data   structures   which   explicitly   specify   the   boundaries   of   regions, the   review   of   hierarchical   data structures was concluded. These structures include Polygonal   Maps   ($\mathbf{PM}$) – a   collection   of   polygons   which   can   either   be vertex-­based   or   edge-­based.   The   vertex-­based   Polygon   Maps   quadtree   or   $PM_1$
 quadtree   in   two­-dimensions   was   due   to Samet \textit{et al}  [30].  
...
r-­sets   which   were   defined   by   Requicha [5].   Lines   and   circles  are   
...
\hspace{30} According   to Weiler [2],   non­-manifold   representations   are   representations   which  
...
\hspace{30} Also, Weiler [2]  developed   the   \textit{\textbf{radial-­edge}}   data   structure   which   can   be  
...
non-­manifold geometric modeling in Karasick [31] and Vanecek [32]. 
