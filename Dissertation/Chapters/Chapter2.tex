...
hence they were called \textit{\textbf{r -­ sets}}
according to \cite{5}. 
\begin{Def2}
...
In a practical sense, it is the skeleton of an object wherein joints are vertices and bones are edges. In \cite{6}, a six ­step algorithm to generate an object's wireframe was developed wherein an object's wireframe was represented by a vertex table and an edge table. 
...
\hspace{30} By looking at data   structures   which   explicitly   specify   the   boundaries   of   regions, the   review   of   hierarchical   data structures was concluded. These structures include Polygonal   Maps   ($\mathbf{PM}$) – a   collection   of   polygons   which   can   either   be vertex-­based   or   edge-­based.   The   vertex-­based   Polygon   Maps   quadtree   or   $PM_1$
 quadtree   in   two­-dimensions   was   due   to   \cite{30}.   The   \textbf{\textit{$PM_1$   quadtree}}   was   based   on   a  
...
\hspace{30} In   \cite{2},   Weiler   developed   the   \textit{\textbf{radial-­edge}}   data   structure   which   can   be  
